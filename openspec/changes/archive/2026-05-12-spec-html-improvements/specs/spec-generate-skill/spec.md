...
インプット収集の直後・ファイル解析の前に、スキルはユーザーに「model.yamlも更新しますか？」と確認しなければならない（SHALL）。ユーザーが不要と回答した場合は、specファイルの生成のみを行う。

#### Scenario: model.yaml更新が不要な場合
- **WHEN** ユーザーがmodel.yaml更新不要と回答する
- **THEN** スキルはspecファイルの生成のみを行い、model.yamlには一切書き込まない

#### Scenario: model.yaml更新が必要な場合
- **WHEN** ユーザーがmodel.yaml更新を希望する
- **THEN** スキルはspecファイルの生成に加え、model.yamlにテーブル定義を追加・更新する
...
- **WHEN** 全テーブルのspecファイルが生成または判定される

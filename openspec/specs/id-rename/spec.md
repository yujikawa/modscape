# id-rename Specification

## Purpose
The system SHALL allow users to rename table and column IDs directly from the DetailPanel, with all cross-schema references updated atomically to preserve model integrity.
...
- **THEN** the system does NOT apply the rename and reverts to the original ID in the input field
...
#### Scenario: Empty column ID
- **WHEN** the user clears a column ID field
- **THEN** the system does NOT apply the rename and reverts to the original ID in the input field

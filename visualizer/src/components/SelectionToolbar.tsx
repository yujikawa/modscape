import { X, Database, Layout, GitGraph, Tag, AlignVerticalJustifyCenter, AlignHorizontalJustifyCenter, Layers, FileChartColumnIncreasing, PanelBottomOpen, PanelBottomClose, StickyNote, ChartLine } from 'lucide-react'
...
const SelectionToolbar = () => {
  ...
  if (!table && !domain && !consumer && !metric && !relationshipData && !annotation && !isMultiSelect) return null
      ...
      <div className={`flex items-center gap-2 border-r pr-4 mr-1 ${theme === 'dark' ? 'border-slate-700' : 'border-slate-100'}`}>
        {isMultiSelect ? (
          ...
          <>
            ...
            {metric && <ChartLine size={16} className="text-emerald-400" />}
        ...
        <div className="flex flex-col leading-tight">
          <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500">
            {isMultiSelect ? 'Multi-Selection' : `Selected ${table ? 'Table' : domain ? 'Domain' : consumer ? 'Consumer' : metric ? 'Metric' : annotation ? 'Annotation' : (((relationshipData?.relationship.type as any) === 'lineage') ? 'Lineage' : 'Relation')}`}
          </span>
          <span className={`text-xs font-semibold truncate max-w-[180px] ${theme === 'dark' ? 'text-slate-200' : 'text-slate-800'}`}>
            {isMultiSelect ? `${selectedTableIds.length} tables selected` : (table ? (table.conceptual?.name ?? table.id) : domain ? domain.name : consumer ? consumer.name : metric ? metric.name : annotation ? (annotation.text?.slice(0, 30) || annotation.id) : relationshipData ? `${relationshipData.relationship.from.table} → ${relationshipData.relationship.to.table}` : '')}
          </span>
        </div>
      </div>

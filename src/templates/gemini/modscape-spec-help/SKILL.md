---
name: modscape-spec-help
description: Display the SDD workflow overview, or answer a specific question about the workflow.
---
...
1. If an argument is provided:
   - If the topic is a **command name** (`requirements`, `design`, `tasks`, `implement`, `archive`, `status`, `amend`, `review`, `search`, `answer`, `validate`, `explain`, `generate`, `note`):
     Read `.gemini/skills/modscape-spec-<topic>/SKILL.md` and provide a user-friendly summary: what it does, what it produces, and when to use it. Do not dump the raw file — summarize for a human reader.

...
4. Build and display the merge preview **before** executing any merge:
   ...
   - **Lineage to replace**: count lineage entries in the main YAML where both `from` and `to` are owned tables in `spec-model.yaml` (isImported !== true) — these will be deleted and replaced by spec-model's lineage (omit row if 0)
   - **Lineage to remove**: explicit deletions from `spec-config.yaml → lineage_to_remove` (omit row if empty)
...
```
If the ID does not exist, display a warning and continue (do not abort):
...
11. Read or create `.modscape/specs/_context.yaml`.
    ...
    **Curation — include only data analysis knowledge:**
    ...
    - Include a `rationale` field when the decision involves a tradeoff
    - Do NOT include: implementation tool choices, team ownership, update schedules, SLA, or deployment procedures
    - Every decision MUST have either `ids: [<entity-ids>]` OR `scope: global`. Decisions that cannot be tagged must NOT be written.
    - For global rules (timezone basis, currency basis, etc.): set `scope: global`
